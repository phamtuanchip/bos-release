    ...
    <el-table :data="model">
      ...
      <el-table-column label="Hành động">
        <template slot-scope="scope">
          <el-button @click="removeLink(scope.$index)"><i class="el-icon-delete"></i></el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      bindEditor() {
        ...
        } else if(this.data.action === "edit-filter") {
          ctrl.action = 'edit';
          ctrl.model = this.$Lodash.cloneDeep(this.data.scope.model.Description.LinkFieldFilter[this.data.item]);
          this.bindTargetFields();
        }
        else
          ctrl.model = {};
      ...
      save() {
        ...
        // } else 
        if(this.data.action === "edit-filter") {
          this.data.scope.bindLinkFieldFilter(this.$Lodash.cloneDeep(ctrl.communication));
        } else 
          this.data.scope.bindLink(this.$Lodash.cloneDeep(ctrl.communication));
        //$mdDialog.hide();
        this.cancel();
