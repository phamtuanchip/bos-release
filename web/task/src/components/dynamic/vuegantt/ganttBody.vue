...
export default {
  ...
  computed: {
    filterGanttData() {
      ...
      if (this.ganttData) return this.ganttData;
      ...
      if (
        !kanban ||
        !Object.keys(kanbankbcList).length ||
        !kanbankbcs ||
        !Object.keys(taskLists).length
      ) {
        return {
          data: []
        };
      ...
      const kbctasklist = kanbankbcs.reduce((acc, kbcid) => {
        ...
        const tasklist = kbcTasks.map(taskid => {
          ...
          return {
            ...
            duration: noStartDue
              ? 366
              : this.getDuration(task.startDate, task.dueDate),
  ...
  watch: {
    ...
    ganttData: { handler: "updateGanttTask", deep: true },
  ...
  methods: {
    ...
    updateGanttTask(val, oldVal) {
      ...
      const isEqual = (arrVal, othVal) =>
        JSON.stringify(arrVal) === JSON.stringify(othVal);
      ...
      if (updatedTask.length === 1) {
        updatedTask.forEach(task => {
          if (task.deleted) {
            ...
            if (!ganttTask) {
              const index = this.kbcTaskList[task.parent].findIndex(
                id => id === task.id
              );
    ...
    getTaskStatus(startDate, dueDate, taskStatus) {
      ...
      const currentDate = Math.abs(
        moment(moment(dueDate).startOf("day")).diff(
    ...
    initiateGantt(data) {
      ...
      gantt.config.touch = this.$isMobileDevice;
    ...
    configGantt() {
      ...
      gantt.config.show_grid = this.isShowTask;
    ...
    templatesGantt() {
      gantt.templates.tooltip_text = (start, end, task) => {
        if (task.type === "tasklist" || task.type === "project")
          return task.text;
        return `<b>Tên công việc:</b> ${task.text}<br/>
                        <b>Tổng ngày：${
                          task.startDate || task.dueDate ? task.duration : ""
                        }</b><br/>
                        <b>Ngày bắt đầu:</b> ${
                          task.startDate
      ...
      gantt.templates.grid_blank = item =>
        `<div class="gantt_hide_item">${item.status}</div>`;
      gantt.templates.task_class = (start, end, task) =>
        task.type === "task"
          ? `${task.status} ${task.type} ${task.startDueStatus}`
          : task.type;

      gantt.templates.grid_row_class = (start, end, task) => {
        switch (task.type) {
          case "project":
            return "gantt_grid_project";
          case "tasklist":
            return "gantt_grid_tasklist";
          default:
            return false;
        }
      };
      ...
      gantt.templates.grid_file = item =>
        `<div class="gantt_label" style="background: ${item.label}"></div>`;
      ...
      gantt.templates.task_text = (start, end, task) => {
        ...
        switch (task.type) {
          ...
          case "tasklist":
            ...
            break;
          case "milestone":
            text += `<div class="gantt_milestone_start-indicator"></div><i class="fa fa-exclamation-triangle"></i><div class="gantt_milestone_end-indicator"></div>`;
            break;
          default:
            // text += `<div class="gantt_task_start-indicator"></div><i class="fa fa-square-o"></i><i class="fa fa-clock-o"></i><div class="gantt_task_end-indicator"></div>`
            break;
        }
        return text;
      };
    ...
    adjustMarkers(top) {
      const markers = [].slice.call(
        document.querySelectorAll(".gantt_marker_content"),
        0
      );
    ...
    attachEventGantt() {
      ...
      this.ganttEvents().on("onGanttScroll", (left, top) =>
        this.adjustMarkers(top)
      );
      ...
      ganttEvents.on("onBeforeDrag", (id, mode, task) => {
        console.log("drag:", id, mode, task);
        return false;
      });
    ...
    attachJqueryEvent() {
      ...
      on(
        $$(
          ".gantt_data_area, .gantt_grid_data, .gantt_row, .gantt_tree_content"
        ),
        "mouseover",
        e => {
    ...
    addMarkerGantt() {
      ...
      gantt.addMarker({
        ...
        css: `today`,
        text: "Hôm nay",
        title: `Ngày： ${moment(today).format("YYYY-MM-DD")}`
      });
    ...
    onShowTaskById(taskId) {
      var task = this.$Lodash.find(this.ganttData.data, { id: taskId });
      if (!task) return;
      if (task.type === "project") return;
      if (task.type === "task") this.showTaskDetail(task);
    }
  },
  mounted() {
    this.$nextTick(() => {
