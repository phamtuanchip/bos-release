      ...
      <div class="col-md-9">
        <el-container class="border-box bg-white">
          <el-main class="p-0">
            ...
            <Pagination v-if="tableData.length>0" :totalPage="totalP" :currentPage="currentP" :pagesize="maxItem" :pagesizes="[10, 25, 50, 100, 1000]" :callback="handleSelectPage" :sizeChanged="handleSizeChange"/>
          </el-main>
        </el-container>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      totalP:0,
      currentP:1,
      maxItem: this.$getItemLocalStorage('PageSize')  ? parseInt(this.$getItemLocalStorage('PageSize')) : 10,
  ...
  methods: {
    handleSizeChange(val){
      this.$setItemLocalStorage('PageSize', val)
      this.maxItem = val;
