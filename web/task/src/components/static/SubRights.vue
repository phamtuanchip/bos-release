    ...
    <el-tabs type="border-card">
      ...
      <el-tab-pane label="Theo kịch bản">
        <el-form label-width="100px">
          <el-form-item label="Phân quyền cho cấp trên:" v-if="activeName !='Project'">
          ...
          <template v-if="activeName =='Project'">
            <el-form-item label="Phân quyền cho cấp trên:">
              <el-select clearable v-model="permissionModel.LeadershipDynamicProject" placeholder="-Chọn cấp-" no-data-text="Không có dữ liệu">
                <el-option v-for="item in CurrentDataGroup.Fields" :key="item.Name" :label="item.Caption"
                           :value="item.Name" v-if="item.Permit"/>
              </el-select>
            </el-form-item>
            <el-form-item label="Trường điều kiện">
              <el-select clearable v-model="permissionModel.LeadershipDynamicCase" placeholder="-Chọn trường-" no-data-text="Không có dữ liệu">
                <el-option v-for="item in CurrentDataGroup.Fields" :key="item.Name" :label="item.Caption"
                           :value="item.Name"/>
              </el-select>
            </el-form-item>
            <el-form-item label="Điều kiện">
              <el-input placeholder="Kích hoạt" v-model="permissionModel.LeadershipDynamicValue"/>
            </el-form-item>
          </template>
        </el-form>
        <el-tabs type="card"  v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="Phòng Ban" name="Group">
            <div class="el-table__body-wrapper is-scroll-left">
              <table style="width: 100%">
                <thead>
                <tr>
          ...
          <el-tab-pane label="Dự án"  name="Project">
            <div class="el-table__body-wrapper is-scroll-left">
              <table style="width: 100%">
                <thead>
                <tr>
                  <th class="text-left" style="width: 30%;">DỰ ÁN</th>
                  <th class="text-center" :style="'width:'+ (70/permissionModel.listRoles.length) +'%;'"
                      v-for="item in permissionModel.listRoles" v-show="item.Name != 'LinkGroup'">
                    {{item.Name}}
                  </th>
                ...
                <tbody>
                <tr v-for="item in permissionModel.listRoleProjects">
                  <td class="text-left">{{item.Caption}}</td>
                  <td class="text-center" v-for="role in permissionModel.listRoles" v-show="role.Name !== 'LinkGroup'">
                    <el-checkbox @change='checkScriptLinked(item.Id,item.Name, role.Code, "project")'
                                 v-model="permissionModel['Script'+item.Name + role.Code + 'project']"/>
                  </td>
                </tr>
                </tbody>
              </table>
...
<script>
  export default {
    ...
    methods: {
      ...
      checkScriptLinked(id, name, code, type) {
        var ctrl = this;
        var exist = false;
        var itemIndex = -1;
        var checkedItem = {};
        ...
        if (!exist) {
          ...
          if ((type && id !== '1' && ctrl.permissionModel['Script' + name + code + type]) || (!type && id !== '1' && ctrl.permissionModel['Script' + id + code + type]) || (id === '1' && ctrl.permissionModel['Script' + name + code + type]))
            checkedItem.KeyCode += code + ';';
            if(type) {
              checkedItem.KeyType = type
            }
          else {
            checkedItem.KeyCode = checkedItem.KeyCode.replace(code + ';', '');
            if (checkedItem.KeyCode === '')
      ...
      bindComboModel() {
        ...
        request = ctrl.$Lodash.cloneDeep(ctrl.$singleRequest);
        request.R1_RequestTemplate = 'Setting.SearchSetting';
        request.R1_ParentCode = 'xSetting.Default.Permissions.RoleProjectTemplate';
        ctrl.$Utils.post(request).then(function (data) {
          ctrl.permissionModel.listRoleProjects = ctrl.$Utils.getDataWithRoot(data.R1, 'Data.DynamicDS.Setting');
          ctrl.signal++;
        })
      ...
      bindPermissionEditor() {
        ...
        $.each(ctrl.permissionModel.passedItem.Description.Permissions, function (k, item) {
          if (item.Permissions !== '') {
            ...
            for (var i = 1; i <= parseInt(objPermissions.Count); i++) {
              ...
              for (var j = 0; j < arrKeyCode.length; j++) {
                ...
                else if(objPermissions['KeyType' + i] == 'project'){
                  ctrl.$set(ctrl.permissionModel,item.Name + objPermissions['Name' + i] + arrKeyCode[j] + 'project', true);
                } else {
          ...
          if (item.Name === 'Static') {
            ...
            if (item.LeadershipDynamicProject !== '' && item.LeadershipDynamicProject !== undefined) {
              ctrl.permissionModel.LeadershipDynamicProject = item.LeadershipDynamicProject;
            }
            if (item.LeadershipDynamicCase !== '' && item.LeadershipDynamicCase !== undefined) {
              ctrl.permissionModel.LeadershipDynamicCase = item.LeadershipDynamicCase;
            }
            if (item.LeadershipDynamicValue !== '' && item.LeadershipDynamicValue !== undefined) {
              ctrl.permissionModel.LeadershipDynamicValue = item.LeadershipDynamicValue;
            }
            // });
    ...
    watch: {
      ...
      CurrentDataGroup: {
        handler: function (after) {
          ...
          this.$set(this.permissionModel, 'listDropdownFields', after.Fields);
        },
        deep: true,
      },
    }
