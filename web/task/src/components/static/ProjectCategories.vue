...
export default {
  ...
  methods: {
    ...
    insertSetting(data) {
      ...
      request.R1_Value = this.data.GroupId;
      request.R1_ValueName = this.data.GroupName;
    ...
    checkParent(selectedItem) {
      if (this.$Utils.isEmpty(selectedItem, "$parent")) {
        ...
        $.each(this.ListTask, (index, task) => {
          if (
            this.$Utils.isEmpty(task, "ProjectCategoriesName") &&
            task.ProjectCategories != ""
          ) {
            var flag = false;
            var newCategory = this.$Lodash.cloneDeep(this.inputData);
            var catName = this.$Utils.isEmpty(task, "ProjectCategoriesName")
              ? task.ProjectCategoriesName.split(";")
              : [];
            ...
            if (flag == true) {
              ...
              taskRequest[
                ...
                "Id;Index;ProjectCategories;Code;";
              taskRequest[
                "R" + [taskRequest.TotalRequests + 1] + "_ProjectCategories"
              ] =
                "";
              taskRequest[
                "R" + [taskRequest.TotalRequests + 1] + "_ProjectCategoriesName"
              ] =
                "";
              ...
              $.each(newCategory, function(val, key) {
                ...
                taskRequest[
                  "R" +
                    ...
                    "_ProjectCategoriesName"
                ] +=
                  val.Name + ";";
                taskRequest[
                  "R" +
                    [taskRequest.TotalRequests + 1] +
                    "_ProjectCategoriesLevel" +
                    [key + 1]
                ] =
                  val.Id;
                taskRequest[
