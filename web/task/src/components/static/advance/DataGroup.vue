            ...
            <el-tab-pane label="Phổ biến" name="0">
              ...
              <el-form :model="model.Description" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
                ...
                <el-form-item label="Code" prop="Code">
                  <el-col :span="11">
                    <el-input v-model="model.Code || model.Description.Code" readonly/>
                  </el-col>
                </el-form-item>
                ...
                <el-form-item label="Nguồn dữ liệu" prop="DataSource">
                  <el-select clearable filterable v-model="model.Description.DataSource" value-key="Id" @change="onDataSourceChange" clearable>
                ...
                <el-collapse v-model="activeNames">
                  ...
                  <el-collapse-item title="Mở rộng các trường phụ thuộc" name="extenField2">
                    ...
                    <el-table :data="model.Description.Dependencies"
                      ...
                      <el-table-column prop="SourceKey"
                                       label="Trường nguồn"
                                        >
                      </el-table-column>
                       <el-table-column prop="TargetKey"
                                       label="Trường đích"
                                      >
                      </el-table-column>
                      ...
                      <el-col :span="11">
                        <!--<el-select v-model="viewModel.LinkDataGroup" value-key="Id" clearable>-->
                          <!--<el-option v-for="item  in viewModel.ListDataGroups" :key="item.Id" :label="item.Caption"-->
                                     <!--:value="item"/>-->
                        <!--</el-select>-->
                        <el-cascader clearable filterable :options="temp" expand-trigger="hover" v-model="viewModel.LinkDataGroup"
                                     placeholder="Chọn" change-on-select/>
                      </el-col>
                    </el-form-item>
                      ...
                      <el-col :span="11">
                        <!--<el-select v-model="viewModel.LinkDataGroup" value-key="Id" clearable>-->
                          <!--<el-option v-for="item  in viewModel.ListDataGroups" :key="item.Id" :label="item.Caption"-->
                                     <!--:value="item"/>-->
                        <!--</el-select>-->
                        <el-cascader clearable filterable :options="temp" expand-trigger="hover" v-model="viewModel.LinkFieldFilter"
                                     placeholder="Chọn" change-on-select/>
                      </el-col>
                    </el-form-item>
                    <el-button @click="excuteLinkFilter($event,'insert', 0)" type="primary">Thêm</el-button>

                    <el-table
                      :data="LinkFieldFilters"
                      style="width: 100%">
                      <el-table-column type="index"
                                       label="Stt"
                                       >
                      ...
                      <el-table-column prop=""
                                       label="Xử lý">
                        <template slot-scope="scope">
                          <el-button @click="excuteLinkFilter($event,'edit',scope.$index)"
                                     size="mini">
                            <i class="el-icon-edit"></i>
                          </el-button>
                          <el-button type="danger" @click="excuteLinkFilter($event,'delete',scope.$index)"
                                     size="mini">
                            <i class="el-icon-delete"></i>
                          </el-button>
                        </template>
  ...
  export default {
    ...
    methods: {
        ...
        utilsLibrary.post(request).then(data => {
          ...
          $.each(ctrl.viewModel.ListDataGroups, (key, dataGroup) => {
            if (dataGroup.Description != undefined && dataGroup.Description != "")
              dataGroup.Description = utilsLibrary.jsonParse(dataGroup.Description) ? utilsLibrary.jsonParse(dataGroup.Description) : alert(dataGroup.Description);
          });
        ...
        if (action === "delete") {
          ...
          utilsLibrary.postCheckResult(request).then(data => {
            ...
            if (currentDataGroup.Description !== undefined) {
              currentDataGroup.Description = utilsLibrary.jsonParse(currentDataGroup.Description);
              ...
              currentDataGroup.Description.Fields = currentDataGroup.Description.Fields !== "" ? utilsLibrary.jsonParse(currentDataGroup.Description.Fields) : [];
              ...
              this.LinkFieldFilters = [];
              if(!ctrl.model.Description.LinkFieldFilter)
                ctrl.model.Description.LinkFieldFilter = {};
              $.each(ctrl.model.Description.LinkFieldFilter, (key, value) => {
                ctrl.LinkFieldFilters.push({ Name: key, Value: value });
              });
              //console.log(ctrl.model.Description  )
              ...
              ctrl.viewModel.ListModules.forEach(item => {
      ...
      excuteSubDirective(ev, type, action, item, index) {
        //console.log(ev, type, action, item, index)
        var ctrl = this;
        ctrl.communication.action = action;
        if (action === "edit") {
          ctrl.communication.item = item;
          if (item.FormJsonExpression !== undefined && item.FormJsonExpression.constructor != String)
        ...
        } else if (type === "link-field-filter") {
          ctrl.communication.action = "edit-filter"
          this.$hub.$emit(
            "set-modal-data",
            "Thông tin các liên kết lọc",
            "60%",
            true,
            "center",
            GroupLinkDialog,
      ...
      excuteLink(ev, action, key) {
        ...
        if (action === "insert") {
          ...
          // if (ctrl.viewModel.LinkFieldFilter && ctrl.viewModel.LinkFieldFilter.length > 0) {
          //   var temp = ctrl.viewModel.LinkFieldFilter[ctrl.viewModel.LinkFieldFilter.length - 1];
          //   if (ctrl.model.Description.LinkFieldFilter[temp] === undefined) {
          //     ctrl.model.Description.LinkFieldFilter[temp] = [];
          //     this.LinkFieldFilters.push({
          //       Name: temp,
          //       Value: []
          //     });
          //   } else {
          //     this.excuteSubDirective(ev, "link-field-filter", "edit", temp);
          //   }
          // }
        } else if (action === "edit"){
      ...
      excuteLinkFilter(ev, action, key) {
        var ctrl = this;
        if (action === "insert") {
          if (ctrl.viewModel.LinkFieldFilter && ctrl.viewModel.LinkFieldFilter.length > 0) {
            var temp = ctrl.viewModel.LinkFieldFilter[ctrl.viewModel.LinkFieldFilter.length - 1];
          ...
          this.excuteSubDirective(ev, "link-field-filter", action, this.LinkFieldFilters[key].Name);
        }
        else if (action === "delete") {
          utilsLibrary
            .showConfirm("Cảnh báo", this.$toastMessage.oneWayAction + this.$toastMessage.deleteSettingWarning)
            .then(() => {
              delete ctrl.model.Description.LinkFieldFilter[this.LinkFieldFilters[key].Name];
              this.Links.splice(key, 1);
            });
        }
      },
      ...
      bindSubDirectives(communication) {
        ...
        if (communication.action === "edit") {
          if (communication.item.FormJsonExpression !== undefined && communication.item.FormJsonExpression !== "" && communication.item.FormJsonExpression.constructor === String) {
            //console.log(communication.item.FormJsonExpression)
      ...
      bindLinkFieldFilter(communication) {
        this.model.Description.LinkFieldFilter[communication.code] = communication.item;
        //console.log(this.model.Description)
        this.LinkFieldFilters.forEach(item => {
          if(item.Name === communication.code){
            item.Value = communication.item;
          }
        })
      ...
      saveModules() {
        ...
        if (utilsLibrary.isEmpty(dataGroup.Description.DataSource))
          dataGroup.Description.DataSource = dataGroup.Description.DataSource.Name;
        ...
        arrModules.forEach((module) => {
          ...
          if (["Form", "Filter", "List"].indexOf(module.Description.ModuleType) !== -1) {
            ...
            module.Description.LinkFieldFilter = dataGroup.Description.LinkFieldFilter;
          }
            ...
            dataGroup.Description.Fields.forEach((dgField) => {
              if (dgField.Name === field.Name) {
                $.each(dgField, (key, value) => {
                  if (!utilsLibrary.isEmpty(field[key]) || field[key] == '')
                    field[key] = value;
                });
                utilsLibrary.deleteAutoValue(field);
              }
          ...
          data = utilsLibrary.getDataWithRoot(data, "UserDS.Group");
            ...
            $.each(data, (key, value) => {
              if (value.GroupType === '0')
                ...
                if (!utilsLibrary.isEmpty(ctrl.viewModel.RolePermissions) || !ctrl.viewModel.RolePermissions[value.GroupParent])
