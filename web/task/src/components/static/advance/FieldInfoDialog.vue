<template>
  <el-form label-width="120px">
    ...
    <el-form-item label="Giao diện">
      <el-select v-model="model.DisplayType" value-key="Code" clearable>
    ...
    <el-form-item>
      <el-checkbox v-model="model.AssignedField" label="Theo phân quyền"></el-checkbox>
  ...
  export default {
    ...
    methods: {
      ...
      save () {
        ...
        if (ctrl.$Utils.isEmpty(savedData.DisplayType)) { savedData.DisplayType = savedData.DisplayType.Code }
        if (ctrl.$Utils.isEmpty(savedData.FormElementType)) { savedData.FormElementType = savedData.FormElementType.Name }

        $.each(savedData, function (key, value) {
          if (value == true && !$.isNumeric(key)) { savedData[key] = 'true' }
          if (value == false && !$.isNumeric(value)) { savedData[key] = 'false' }
