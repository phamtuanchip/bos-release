<template>
  <el-form>
    ...
    <el-checkbox v-model="model.DisplayTable">Hiển thị bảng (Dashboard)
    </el-checkbox>
    <el-checkbox v-model="model.HideReport">Ẩn biểu đồ  (Dashboard)
    </el-checkbox>
    ...
    <el-form-item label="Trường">
      <el-select v-model="viewModel.Key" filterable>
        <el-option :value="item.Name" v-for="item in viewModel.Fields" :label="item.Caption + '('+ item.Name + ')'">
  ...
  export default {
    ...
    methods: {
      save() {
        ...
        if (ctrl.data.scope.model.Description.IsDashboard)
          ctrl.model.DefaultFilter[ctrl.data.item.$parent.Caption] = ctrl.viewModel.ListFilters;
        else
        ...
        if (this.data.action == 'edit') {
          ...
          if(ctrl.data.item.$parent){
            $.each(ctrl.data.item.$parent.children, (i, v)=>{
              if(ctrl.model.ModuleCode == v.ModuleCode){
                ctrl.data.item.$parent.children.splice(i, 1);
                ctrl.data.item.$parent.children.splice(i, 0, ctrl.model);
                ctrl.$hub.$emit('reloadFieldTree');
                return false;
              }
            });
          ...
          ctrl.data.item.$parent.children.push(ctrl.model);
    ...
    created() {
      ...
      if (ctrl.data.action == 'edit') {
        ...
        if (ctrl.data.scope.model.Description.IsDashboard) {
          if (ctrl.model.DefaultFilter[ctrl.data.item.$parent.Caption] == undefined){
            ctrl.$set( ctrl.model.DefaultFilter, ctrl.data.item.$parent.Caption, []);
          }
          ctrl.viewModel.ListFilters = ctrl.model.DefaultFilter[ctrl.data.item.$parent.Caption];
        } else {
