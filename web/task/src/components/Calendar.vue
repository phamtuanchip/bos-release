      ...
      events: function (newEvents) {
        ...
        if(self.isMobile){
          self.$parent.eventList = self.cal.fullCalendar('clientEvents', (event) => {
            return event.start.format("DD-MM-YYYY") === self.$parent.dateText || (event.end && event.start.format("DD-MM-YYYY") <= self.$parent.dateText && event.end.format("DD-MM-YYYY") >= self.$parent.dateText);
          });
        }
      },
      ...
      if (self.isMobile) {
        this.$set(args, 'eventLimitClick', (cellInfo, jsEvent) => {
          ...
          self.$parent.eventList = self.cal.fullCalendar('clientEvents', function (event) {
            return event.start.format("YYYY-MM-DD") === Vue.moment(cellInfo.date).format("YYYY-MM-DD") || (event.end && event.start.format("YYYY-MM-DD") <= Vue.moment(cellInfo.date).format("YYYY-MM-DD") && event.end.format("YYYY-MM-DD") >= Vue.moment(cellInfo.date).format("YYYY-MM-DD"));
          });

          self.$parent.dateText = Vue.moment(cellInfo.date).format("DD-MM-YYYY");
        })
      }
      ...
      this.$on('event::dropped', event => {
        ...
        if (this.$rootScope.isGoogleCalendar) {
          ...
          var request = {
            ...
            // R1_FormSetting: vm.SettingForm.length == 0 ? undefined : JSON.stringify(vm.SettingForm),
          };
          var sourceData = this.$Lodash.cloneDeep(event.task);
          var sourceSetting = vm.$Lodash.cloneDeep(vm.SettingForm);
          vm.$Utils.replacePermission(sourceSetting, sourceData)
    ...
    methods: {
      ...
      searchTask(start, end) {
        ...
        var requestTask = {
          ...
          R1_RequestTemplate: 'AG_Task_Task.Search',
          R1_RequestDataGroup: 'DataGroup.danh-sach-cong-viec.04b66',
          R1_RequestFields: 'Id;Index;Deadline;Name;PlanStartDate;PlanEndDate;Worker;VersionProject;Status;WorkerName;VersionProjectName;GroupRoot;Manager;Subscribe;GroupPermission;',
          R1_AssignedUser: this.$getItemLocalStorage(this.$localStorageConstant.SessionId),
          R1_PlanStartDateStartValue: Vue.moment(start).format('YYYY-MM-DD') + 'T00:00:00',
          R1_PlanStartDateEndValue: Vue.moment(end).format('YYYY-MM-DD') + 'T23:59:59',
        };
      ...
      loadSetting() {
        ...
        vm.$Utils.post(request).then(data => {
          vm.SettingForm = vm.$Utils.getDataWithRoot(
            data.R1,
            "Data.DynamicDS.Setting"
          )[0];
          vm.$Utils.formatFormSetting(vm.SettingForm)
        });
      },
    },
    watch: {
