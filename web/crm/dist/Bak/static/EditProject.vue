<template>
  <div class="EditProject">
    <el-form :model="model">
      ...
      <el-row>
        <el-col :span="11">
          <el-form-item  label="Giám sát" >
            <el-select v-model="model.Lead" placeholder="Giám sát" style="width: 100%" multiple :collapse-tags="$isMobileDevice">
              <el-option v-for="item in ListUser"
                         :key="item.UserId"
                         :label="item.Username"
                         :value="item.UserId">
  ...
  methods: {
    ...
    save() {
      ...
      if (saveAble) {
        ...
        var Lead = this.$Lodash.cloneDeep(this.model.Lead);
        this.model.LeadName = "";
        this.model.Lead = "";
        Lead.forEach(value => {
          var objLead = this.ListUser.filter(el => {
            return el.UserId == value;
          });
          this.model.LeadName += objLead[0].Username + ";";
          this.model.Lead += value + ";";
        });
    ...
    UpdateProject(group) {
      ...
      request.R1_RequestDataGroup = "DataGroup.quan-ly-du-an.053gf";
      request.R1_RequestFields =
        "Id;Code;CreatedByName;ModifiedByName;Permit;PermitName;Status;StatusName;Active;ActiveName;Name;Describe;ProjectCode;Department;DepartmentName;Manager;ManagerName;Worker;WorkerName;Lead;LeadName;";
      request.R1_Code = "Project";
      ...
      this.$Utils.post(request).then(data => {
        ...
        if (data.success) {
          ...
          RoleLeader.forEach(code => {
            ...
            this.model.Lead.split(";").forEach(lead => {
              if (lead != "") {
                var linkEmail = {
                  RequestClass: "xBase",
                  Parent: data.R1.Data,
          ...
          if (
            this.$Utils.isEmpty(this.data.user, "Lead") &&
            this.data.user.Lead != "" &&
            this.model.Lead != this.data.user.Lead
          ) {
            this.data.user.Lead.split(";").forEach(user => {
              if (user != "" && this.model.Lead.indexOf(user) == -1) {
                var paramsSearchLinked = {
                  RequestAction: "SearchLinked",
                  RequestClass: "Addition",
                  ConditionFields: "Parent;Child;",
  ...
  created() {
    ...
    this.$Utils.post(request).then(data => {
      ...
      data.forEach(item => {
        ...
        if (this.$Utils.isEmpty(Lead) && Lead.indexOf(item.UserId) > -1) {
          if (
            (!this.$isSystemAdmin() && item.LoginName != "superadmin") ||
            this.$isSystemAdmin()
          ) {
            this.model.Lead.push(item.UserId);
          }
        }
        if (
          (!this.$isSystemAdmin() && item.LoginName != "superadmin") ||
