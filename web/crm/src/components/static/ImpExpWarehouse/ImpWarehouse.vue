<script>
  export default {
    ...
    methods: {
      loadResources(){
        ...
        this.$Utils.post(requestWH).then(response => {
          this.params.warehouses = this.$Utils.getDataWithRoot(response.R1, 'Data.DocumentDS.Document');
          ...
          this.$Utils.post(request).then(response => {
            this.params.poses = this.$Utils.getDataWithRoot(response.R1, 'Data.DocumentDS.Document');
            $.each(this.params.warehouses, (k,value)=> {
              if(value.Id == this.params.poses[0].Category){
                this.receiptModel.SourceUrl = value;
                return false;
              }
            });
          });
        ...
        this.$Utils.post(request).then(response => {
          this.params.sources = this.$Utils.getDataWithRoot(response.R1, 'Data.DynamicDS.Setting');
        })
        ...
        this.isMobile = this.$isMobileDevice;
        this.$Utils.post(request).then(response => {
          this.params.campaignCodes = this.$Utils.getDataWithRoot(response.R1, 'Data.DynamicDS.Setting');
        })
        ...
        this.isMobile = this.$isMobileDevice;
        this.$Utils.post(request).then(response => {
          this.params.timeFrameNames = this.$Utils.getDataWithRoot(response.R1, 'Data.DynamicDS.Setting');
        })
        ...
        this.isMobile = this.$isMobileDevice;
        this.$Utils.post(request).then(response => {
          this.params.genders = this.$Utils.getDataWithRoot(response.R1, 'Data.DynamicDS.Setting');
        })
        ...
        this.isMobile = this.$isMobileDevice;
        this.$Utils.post(request).then(response => {
          this.params.provinces = this.$Utils.getDataWithRoot(response.R1, 'Data.DynamicDS.Setting');
        })
      ...
      getFormSetting(type, formCode){
        ...
        this.$Utils.post(req).then((response)=>{
          ...
          this.formSetting[type] = JSON.stringify(sourceSetting[0].Description);
          this.fields[type] = this.$Utils.rotateDementionArr(sourceSetting[0].Description.Fields, 'Name');
        });
      },
      handleBarcodeSearchConfirm () {
        if(this.barcode_search_keyword == '') {
          // this.$message({message: 'Chưa điền barcode', center: true, type: 'error'});
        }
        else if(this.$Utils.equals(this.receiptModel.SourceUrl,{})){
          this.$message({message: 'Phải chọn kho hàng trước',center: true,type: 'error'});
        }else {
          var barcodeSearched = this.$Lodash.cloneDeep(this.barcode_search_keyword);
          if(this.cachedBarcodes.indexOf(barcodeSearched) >-1){
            ...
            var request = {
              ...
              R1_Status: this.receiptModel.SourceUrl.Id,
              R1_RequestTemplate: "xDocument_Document.Search",
            ...
            this.$Utils.post(request).then(response => {
              ...
              if(productLst && productLst.length ==1){
                ...
                if(!existed){
                  ...
                  this.cart.push(productLst[0]);
                  this.cachedBarcodes.push(barcodeSearched);
                }
                ...
                var requestInsertWH = {
                  ...
                  R1_RequestFields: 'Code;MetaKeywords;OfficialNumber;Name;ProductCode;Lead;MetaDescription;MetaTitle;Signer;Title;Source;ProductColorName;SizeName;TotalPages;Priority;TotalAttachments;Status;StatusName',
                }
                $.each(product, (k, v)=> {
                  requestInsertWH['R1_'+k] = v;
                });
                if(this.receiptModel.StatusType.Id=='515a2438-5274-4d1f-9fa6-7dbee94c524b'){//xuất
                  requestInsertWH['R1_Status'] = this.receiptModel.Source.Id;
                  requestInsertWH['R1_StatusName'] = this.receiptModel.Source.Name;
                }else{//nhập
                  requestInsertWH['R1_Status'] = this.receiptModel.SourceUrl.Id;
                  requestInsertWH['R1_StatusName'] = this.receiptModel.SourceUrl.Name;
                }
        ...
        this.$refs['receiptForm'].validate((valid) => {
          if(valid){
            var requestReceipt = {
              ...
              RequestAction: 'Request',
              TotalRequests: 1,
              R1_RequestDataGroup: 'DataGroup.nhap-xuat-kho-theo-phieu.0132n',
              R1_RequestTemplate: 'xDocument_Document.Insert',
              ...
              R1_FormSetting: this.formSetting.import,
            }
            var requestUpdateWH = {
              RequestClass: 'xBase',
              RequestAction: 'Request',
              TotalRequests: this.cart.length,
            }
            ...
            $.each(this.cart, (k,val)=> {
              requestReceiptProduct['R'+ (k+1) +'_Changed'] = JSON.stringify(receiptChanges);
              requestReceiptProduct['R'+ (k+1) +'_SourceUrl']= this.receiptModel.SourceUrl.Id;
              requestReceiptProduct['R'+ (k+1) +'_SourceUrlName']= this.receiptModel.SourceUrl.Name;
              requestReceiptProduct['R'+ (k+1) +'_Source']= this.receiptModel.Source.Id;
              requestReceiptProduct['R'+ (k+1) +'_SourceName']= this.receiptModel.Source.Name;
              requestReceiptProduct['R'+ (k+1) +'_StatusType'] = this.receiptModel.StatusType.Id;
              requestReceiptProduct['R'+ (k+1) +'_StatusTypeName'] = this.receiptModel.StatusType.Name;
              requestReceiptProduct['R'+ (k+1) +'_StoreCode']= this.receiptModel.StoreCode;
              requestReceiptProduct['R'+ (k+1) +'_ReferenceNumber'] =  this.receiptModel.ReferenceNumber;
              requestReceiptProduct['R'+ (k+1) +'_RelatedDocuments'] =  this.receiptModel.RelatedDocuments;
              requestReceiptProduct['R'+ (k+1) +'_ExpiredDate'] =  this.$Utils.formatDateTime(this.receiptModel.ExpiredDate);
              ...
              requestReceiptProduct['R'+ (k+1) +'_FormSetting']= this.formSetting.importProduct;
              if(this.receiptModel.StatusType.Id=='515a2438-5274-4d1f-9fa6-7dbee94c524b')//xuất
              ...
              requestUpdateWH['R'+ (k+1) +'_FormSetting'] = this.formSetting.warehouse;
            })

            var receiptChanges = [];

            $.each(this.receiptModel, (k,v)=> {
              if(k == 'Name' || k.indexOf('Name') != k.length-4){
                var change = {
                  "ColumnCaption": this.fields['import'][k].Caption,
                  "ColumnName": k,
                  "OldDisplayValue": '',
                  "OldValue": '',
            ...
            requestReceipt.R1_SourceUrl= this.receiptModel.SourceUrl.Id;
            requestReceipt.R1_SourceUrlName= this.receiptModel.SourceUrl.Name;
            requestReceipt.R1_Source= this.receiptModel.Source.Id;
            requestReceipt.R1_SourceName= this.receiptModel.Source.Name;
            requestReceipt.R1_StatusType = this.receiptModel.StatusType.Id;
            requestReceipt.R1_StatusTypeName = this.receiptModel.StatusType.Name;
            requestReceipt.R1_StoreCode= this.receiptModel.StoreCode;
            requestReceipt.R1_ReferenceNumber =  this.receiptModel.ReferenceNumber;
            requestReceipt.R1_RelatedDocuments =  this.receiptModel.RelatedDocuments;
            requestReceipt.R1_ExpiredDate =  this.$Utils.formatDateTime(this.receiptModel.ExpiredDate);

            this.$Utils.post(requestReceipt).then(respond=> {
              if(respond.success){
                $.each(this.cart, (k,val)=> {
                  requestReceiptProduct['R'+(k+1)+'_Parent'] = respond.R1.Data;
                });
                this.$Utils.post(requestReceiptProduct).then(respond=> {
                });
              }
            });
            this.$Utils.post(requestUpdateWH).then(respond=> {
            });
    ...
    created: function (){
      ...
      this.getFormSetting('importProduct', 'Form.phieu-nhap-kho.0fbc0');
      // this.getFormSetting('export', 'Form.xuat-luan-chuyen.075dg');
    }
  }
</script>
<template>
        ...
        <div class="step__wrapper">
          <el-card>
            <div class="Cart__ROOT">
              ...
              <div class="cart__content" style="padding: 10px;">
                <el-form :model="receiptModel" :inline="true" label-position="top" ref="receiptForm">
                  <el-form-item label="Ngày nhập">
                    <el-date-picker format='dd/MM/yyyy' v-model="receiptModel.ExpiredDate"></el-date-picker>
                  </el-form-item>
                  <el-form-item label="Nhập từ">
                    <el-select v-model="receiptModel.Source" value-key="Id" placeholder="Chọn kho hàng" filterable>
                      <el-option v-for="item in params.warehouses" :key="item.Id" :label="item.Name" :value="item"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="Nhập đến:">
                    <el-select v-model="receiptModel.SourceUrl" value-key="Id" placeholder="Chọn kho hàng" filterable>
                      <el-option v-for="item in params.warehouses" :key="item.Id" :label="item.Name" :value="item"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="Mã phiếu">
                    <el-input v-model="receiptModel.StoreCode"></el-input>
                  </el-form-item>
                  <el-form-item label="Mã phiếu liên quan">
                    <el-input v-model="receiptModel.ReferenceNumber"></el-input>
                  </el-form-item>
                  <el-form-item label="Loại">
                    <el-select disabled v-model="receiptModel.StatusType" value-key="Id" placeholder="Loại" filterable>
                      <el-option v-for="item in params.statusType" :key="item.Id" :label="item.Name" :value="item"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="Ghi chú"  style="width: 50%">
                    <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 4}" v-model="receiptModel.RelatedDocuments"></el-input>
                  </el-form-item>
          ...
          <el-card>
            <div class="Cart__ROOT">
              <div class="cart__header">
                ...
                <div class="col-lg-6 pd0">
                  <el-input placeholder="Nhập Barcode và Enter" autofocus suffix-icon="el-icon-search" v-model="barcode_search_keyword" @keyup.enter.native="handleBarcodeSearchConfirm"></el-input>
                </div>
                <div class="clearfix"></div>
              </div>
              <div class="cart__content">
                <el-table :data="cart" style="width: 100%" empty-text="Chưa có sản phẩm">
                  <el-table-column type="index" width="40" align="center" header-align="center"></el-table-column>
                  <el-table-column prop="MetaKeywords" label="Barcode"></el-table-column>
                  <el-table-column prop="ProductCode" label="Mã sản phẩm in trên tem mác"></el-table-column>
                  ...
                  <el-table-column label="Hành động" fixed="right" v-if="$isMantisAdmin()|| $isSystemAdmin()">
                    <template slot-scope="scope">
                      <el-button @click="handleEditProduct(scope.row.Id)">
                        <i class="fa fa-pencil"></i>
                      </el-button>
                      <el-button type="danger" style="margin: 3px 0;" @click="handleDeleteProduct(scope.row.Id)">
          ...
          <div class="Cart__ROOT">
            <div class="cart__header">
              <div class="col-md-2 offset-md-10">
                <el-button type="danger" class="fa fa-save"  @click="save">
                  Lưu
                </el-button>
              </div>
            </div>
