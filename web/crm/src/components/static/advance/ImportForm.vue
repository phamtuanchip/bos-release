    ...
    methods: {
      ...
      bindDataGroupSource() {
        ...
        utilsLibrary.post(request).then(data => {
          ...
          ctrl.viewModel.ListDataGroups = this.$Lodash.cloneDeep(data);
          $.each(ctrl.viewModel.ListDataGroups, (key, item) => {
              item.Description =
              (item.Description && item.Description.constructor === String) ? JSON.parse(item.Description) : item.Description;
            item.Fields = "";
            ...
            try {
            item.Fields = (item.Fields && item.Fields != "" && item.Fields.constructor === String) ? JSON.parse(item.Fields) : [];
            } catch (error) {
              console.error(error)
            }
          });
        ...
        utilsLibrary.postCheckResult(request).then(data => {
          ...
          $.each(arrFields, (key, childNode) => {
            ...
            $.each(ctrl.model.Columns, (key, column) => {
              //if(column) column = column.trim();
              var caption = ctrl.$Lodash.cloneDeep(column).trim();
              if (caption === childNode.Caption) {
          ...
          if (data.success || request.TotalRequests === 0) {
            ...
            $.each(ctrl.model.Rows, (index, excelRow) => {
              ...
              $.each(ctrl.viewModel.Convert.listColumns, (key, column) => {
                ...
                if(excelValue) excelValue = excelValue.trim()
      ...
      excuteImportRequest(selectedDataGroup, arrAdditionConditions, strRequestFields) {
        ...
        var startIndex = ctrl.viewModel.Loop;
        var endIndex = startIndex + eval(ctrl.viewModel.NumberRequestPerTime);
        ctrl.viewModel.Loop = endIndex > ctrl.viewModel.Convert.listRows.length ? ctrl.viewModel.Convert.listRows.length : endIndex;
        endIndex = ctrl.viewModel.Loop;
        var index = 0;
          ...
          // if ((i !== 0 && index === mod - 1) || (i === ctrl.viewModel.Convert.listRows.length - 1)) {
            ...
            utilsLibrary.post(request).then(data => {
              if (data.success) {
                ...
                if (ctrl.viewModel.listLogs.length > 0) {
                  var prelog = ctrl.viewModel.listLogs[ctrl.viewModel.listLogs.length - 1];
                  log.Success = prelog.Success;
                  log.Inserted = prelog.Inserted;
                  log.Updated = prelog.Updated;
                  log.Failed = prelog.Failed;
                }
                ...
                if (ctrl.viewModel.Loop < ctrl.viewModel.Convert.listRows.length) {
                  ctrl.excuteImportRequest(selectedDataGroup, arrAdditionConditions, strRequestFields);
                } else {
                  utilsLibrary.showMessage('success', ctrl.$toastMessage.updateItemPre + ctrl.$toastMessage.updateItemSuf);
                  // dataToPass.item.isImported = true;
                }
              } else
                utilsLibrary.showMessage(
                  "error",
