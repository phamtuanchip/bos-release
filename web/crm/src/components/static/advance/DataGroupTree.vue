<template>
  <div class="data-group">
    <b-card>
      ...
      <sortable-tree :data="data" mixinParentKey="$parent" @changePosition="changePosition" :dragEnable="true" closeStateKey="$foldClose">
        <template slot-scope="{item}">
          <div class="custom-tree-content" :class="{'exitChild': item.children && item.children.length}">
            ...
            <span  :class="'child-' + item.Level" class="mr-3">
              <a href="javascript:void(0)" v-show="item.Level !== '3'" @click="editCategory(item)">{{item.Name}}</a>
              <a href="javascript:void(0)" v-show="item.Level === '3'">{{item.Name}}</a>
            </span><span v-if="item.level!='0'">
            <a href="javascript:void(0)" @click="addCategory(item)"><i class="fa fa-plus" aria-hidden="true"></i></a> <span v-show="item.Level !== '3'">|</span>
              <!--<a href="javascript:void(0)" v-show="item.Level !== '3'" @click="editCategory(item)"><i class="el-icon-edit" aria-hidden="true"></i></a> <span v-show="item.Level !== '3'">|</span>-->
              <a href="javascript:void(0)" v-show="item.Level !== '3'" @click="confirmToDelete(item)"><i class="fa fa-trash" aria-hidden="true"></i></a>
            </span>
          </div>
        </template>
      </sortable-tree>
...
<script>
  ...
  import CreateDataGroupForm from '@/components/static/advance/CreateDataGroupForm'
  Vue.component(SortableTree.name, SortableTree)
  export default {
    props: ['data'],
    components: { CreateDataGroupForm },
    data() {
      return {
        Name: '',
        Parent: '',
    ...
    methods: {
      init() {
        var request = {
          ...
          R1_ParentCode: 'xSetting.Project.DataGroup',
        ...
        this.$Utils.postCheckResult(request).then(data => {
          ...
          //this.ListData = arrPages;
          this.treeData.children = this.$Utils.createCustomTreeFromList(
            this.ListData,
            "Id",
            "ParentId",
            "children",
      ...
      confirmToDelete(data){
         this.$confirm(
        "Bạn có chăc chắn muốn xóa nghiệp vụ {0} này?".replace('{0}', data.Name),
        "Cảnh báo",
        {
