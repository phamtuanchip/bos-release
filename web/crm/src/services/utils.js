...
export default {
  ...
  URL() {
     return '';
     // return 'http://test5.mn.com.vn';
    // return 'http://test2.mn.com.vn';
    // return 'http://test3.mn.com.vn';
      ...
      case 'Double':
        if (value != '' && !isNaN(value)) {
          ...
          if (setting.FormatValue) {
            ...
            value = (value == 0) ? 0 : format(parseFloat(value).toFixed(afterDecimal), afterDecimal);
            formated = true;
          }
  ...
  getDateTimeByKey(key, format, dateOption, backDate) {
    ...
    if (backDate != undefined) {
      date = Vue.moment(date).subtract(backDate, 'days').toDate();
    }
    switch (key) {
      case 'DayAgoStart':
    ...
    $.each(sourceSetting.Permissions, (key, value) => {
      if (this.isEmpty(value.Permissions)) {
        sourceSetting.Permissions[
          key
        ].Permissions = this.stringToObject(value.Permissions);
