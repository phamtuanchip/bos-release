<template>
  <div class="dynamic-page">
    ...
    <div v-if="Setting.IsDashboard =='true'" class="row">
      <div v-for="(moduleGroup, key) in Setting.Groups" :class="'dynamicDashboard col-lg-'+parseInt(moduleGroup.Flex /100 * 12) + ' col-md-'+parseInt(moduleGroup.FlexSM /100 * 12) + ' col-sm-'+parseInt(moduleGroup.FlexXS /100 * 12)" v-if="!$isMobileDevice">
        <el-card>
      ...
      <div class="col-md-12" v-if="$isMobileDevice">
        <el-tabs v-model="activeName" >
          <el-tab-pane v-for="(moduleGroup, key) in Setting.Groups" :label="moduleGroup.Caption" :name="key" class='text-center'>
            <el-card v-if="activeName == key">
              <div v-for="pModule in moduleGroup.Modules" :key="pModule.ModuleCode" v-if="!reportPage && pModule.ModuleType == 'Filter' && pModule.Fields && pModule!=undefined" class="row">
                <DynamicFilter :pSetting="pModule"></DynamicFilter>
              </div>
              <div v-for="pModule in moduleGroup.Modules" :key="pModule.ModuleCode" v-if="!reportPage">
...
export default {
  ...
  methods: {
    showField (data) {
      // if(viewModel.ShowField[data.Name] != 'true'){
      //   viewModel.ShowField[data.Name] = 'true';
      // } else {
      //   viewModel.ShowField[data.Name] = 'false';
      // }
    },
    callback() {},
    // moduleReady() {
      ...
      ctrl.checkSettingPage().then(() => {
        ...
        $.each(ctrl.Setting.Module, (key, val) => {
          if (ctrl.$Utils.isEmpty(val, 'DefaultFilter')) {
            ...
            //   if(!ctrl.Setting[val.ModuleCode + 'AdditionConditions']){
            //     ctrl.Setting[val.ModuleCode + 'AdditionConditions'] = {};
            //   }
            //   ctrl.Setting[val.ModuleCode + 'AdditionConditions'][val.Group] = val.DefaultFilter;
            // } else
        ...
        if (ctrl.$Utils.isEmpty(ctrl.Setting.Groups)) {
          ctrl.Setting.Groups.forEach((listItem) => {
            ctrl.Setting.Module.forEach((module) => {
              ...
              // if(ctrl.Setting[module.ModuleCode + 'AdditionConditions'] && ctrl.Setting[module.ModuleCode + 'AdditionConditions'][module.Group] && ctrl.Setting[module.ModuleCode + 'AdditionConditions'][module.Group][listItem.Caption]) {
              //   ctrl.Setting[module.ModuleCode + 'AdditionConditions'][module.Group] = ctrl.$Lodash.cloneDeep(ctrl.Setting[module.ModuleCode + 'AdditionConditions'][module.Group][listItem.Caption])
              // }
