<template>
      ...
      <div class="el-table__body-wrapper grab-bing" v-dragscroll>
        <table>
          ...
          <tr>
            ...
            <th class="default-column" v-if="datasrc.ListCols['Kế hoạch']" :colspan="datasrc.ListCols['Kế hoạch'].length">Kế hoạch</th>
          ...
          <tr :class="cell.CellType " v-for="(cell, cIndex) in datasrc.cells" :style="cell.CellType == 'h'?'background-color:#f50808 ; color: white; font-weight:bold; font-size: 15px': ''" v-if="cell.Display">
 ...
 <script>
   ...
   export default {
     ...
     data() {
       return {
        // functions: {'onShowMenu': this.onShowMenu}
       };
     },
     methods:{
     ...
     created() {
      // if(this.datasrc && this.datasrc.cells && this.datasrc.cells.length > 0)
      //   this.onShowMenu(new Date().getDay(), true);
     }
