<template>
 <div>
   <el-row>
     ...
     <el-radio v-if="isSystemAdmin" @change="chooseSelect" size="mini" v-model="radio" label="2">Vai trò người dùng</el-radio>
     <!-- <router-link to="/settings/xSetting.Default.Permissions.RoleTemplate"> -->
      <el-radio v-if="isSystemAdmin" @change="chooseSelect" size="mini" v-model="radio" label="3">Vai trò phòng ban</el-radio>  
      <!-- </router-link> -->
   </el-row>
   <Parameter settingType="rolePermission" v-if="this.radio === '3'" class="mgt10"></Parameter>
   <GroupTreeGrid
     :datasource="groups"
     :group="groupP"
     :checkRadio="radio"
     :title="title"
     v-if="this.radio !== '3' && this.radio !== '4'"
   />
...
export default {
  components: { CreateGroupForm, GroupTreeGrid, Parameter },
  data() {
    return {
      radio: "1",
        ...
        response => {
          if (response.success) {
            ...
            var groupRoleTemplate = [];
                ...
                if (data) {
                  group = group.filter(function(el) {
                    return (
