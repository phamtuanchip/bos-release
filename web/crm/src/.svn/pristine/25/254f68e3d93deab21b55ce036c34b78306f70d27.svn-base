<template>
  <div>
    <el-form ref="form" :model="emailForm" label-position="top">
      <el-form-item label="Người nhận">
         <el-select multiple v-model="receiverEmail" placeholder="Người nhận" style="width: 100%;">
      ...
      <el-form-item label="Hoặc email (cách nhau bằng dấu ;)">
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="closeForm()">Bỏ qua</el-button>
      <el-button @click="SendMessage()" type="primary">Gửi</el-button>
    </span>
  </div>
</template>
    ...
    methods: {
      loadData() {
        ...
        this.$Utils.post(request).then((user) =>{
          ...
          this.worker = userData.filter(user=>{
            return (this.$isSystemAdmin() || (!this.$isSystemAdmin() && user.LoginName.toLowerCase() != 'superadmin'))
          });
          this.senderEmail = this.loggedOnUser.Email //= userData[0].Email;
        })
        var content = "";
        var content = "";
      ...
      SendMessage() {
        ...
        var cc = this.ccEmail.replaceAll(',', ';').replaceAll(' ', '').split(';')
        if(this.receiverEmail.length == 0 || cc == 0) {
           this.$Utils.showMessage('error', 'Phải chọn người nhận hoặc nhập email');
          return
        }
        this.receiverEmail = this.receiverEmail.concat()
        var to = this.receiverEmail.join(';');               
        var requestSendmail = this.$Lodash.cloneDeep(this.$singleRequest);
        requestSendmail.RequestTemplate = "SendMessage";
        requestSendmail.R1_To = to;
