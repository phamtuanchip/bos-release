<template>
  <el-table :data="tableData.cellData" :stripe="tableData.stripe" :border="tableData.border"
            :style="'width:'+tableData.width" :empty-text="tableData.emptyText" highlight-current-row @selection-change="handleSelectionChange">>
    ...
    <el-table-column v-for="column in tableData.columns" :label = "column.label" :type="column.type"
                     :fixed="column.fixed" >
      <template v-if="!column.type" slot-scope="scope">
        ...
        <div v-else :style="column.style + ' ;color:'+ tblData.colorifyText(scope.row, column)">{{scope.row[column.name]}}</div>
      </template>
    </el-table-column>
  </el-table>
</template>
