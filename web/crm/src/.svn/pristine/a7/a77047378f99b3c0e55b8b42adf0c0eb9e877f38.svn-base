<template>
  <div>
    <el-form>
      <el-tabs type="border-card">
        ...
        <el-tab-pane v-if="model.UserId" label="Thông báo">
           <table class="w-100">
                  <thead>
                  <tr class="row" >
                    <th  class="col-6">Phần công việc</th>
                    <th  class="col-6">Phần tài liệu</th>
                  </tr>
                  </thead>
                  <tbody>
                  <tr class="row" style="border-bottom: 1px dotted lightblue !important" >
                    <td class="col-6">
                       <div class="row">
                        <div class="col">
                            <label>Tất cả</label>                          
              ...
              <el-card>
                <table class="w-100">
                  ...
                  <tr class="row" style="border-bottom: 1px dotted lightblue !important" v-for="item in Departments" :key="item.GroupId">
                    <td class="col-6">
                      <el-checkbox v-model="viewModel.Groups[item.GroupId]" @change="checkGroup($event,item)"
                    ...
                    <td class="col-6">
                        ...
                        <div class="col-md-6 col-sm-12 col-xs-12" v-for="grole in item.children" :key="grole.GroupId" >
                          <el-checkbox @change="handleRoleChange($event, item, grole.GroupId)" v-model="viewModel.GroupRoles[grole.GroupId]"
            ...
            <div class="col-md-6 col-sm-12 col-xs-12" id="tblGroups">
              <el-card>
                <table class="w-100">
                  <thead>
                  <tr class="row" >
                    <th  class="col-6">Dự án</th>
                    <th  class="col-6">Vai trò</th>
                  </tr>
                  </thead>
                  <tbody>
                  <tr class="row" style="border-bottom: 1px dotted lightblue !important" v-for="item in Project" :key="item.GroupId">
                    <td class="col-6">
                      <el-checkbox v-model="viewModel.Groups[item.GroupId]" @change="checkGroup($event, item)"
                                   :disabled="!isSystemAdmin && !isMantisAdmin">
                        {{item.GroupName}}
              ...
              <el-card>
                <table class="el-table__body-wrapper is-scroll-left">
                  ...
                  <tr v-for="item in Project " :key="item.Id">
                    <td>
                      <el-checkbox v-model="viewModel.Projects[item.GroupId]" @change="checkProject($event, item, true)"
                                   :disabled="!isSystemAdmin && !isMantisAdmin">
                        {{item.GroupName}}
                      </el-checkbox>
                    </td>
                    <td class="row">
                      <div class="col-md-6 col-sm-12 col-xs-12">
                        <el-checkbox v-model="viewModel.WProject[item.GroupId]" @change="checkRoleProject($event, item, viewModel.MProject[item.GroupId], true)"
                                     :disabled="!isSystemAdmin && !isMantisAdmin">
                          Thành viên
                        </el-checkbox>
                      </div>
                      <div class="col-md-6 col-sm-12 col-xs-12">
                        <el-checkbox v-model="viewModel.MProject[item.GroupId]" @change="checkRoleProject($event, item, viewModel.WProject[item.GroupId], false)"
                                     :disabled="!isSystemAdmin && !isMantisAdmin">
                          Trưởng dự án
                        </el-checkbox>
                      </div>
                      <div class="col-md-6 col-sm-12 col-xs-12">
                        <el-checkbox v-model="viewModel.LProject[item.GroupId]" @change="checkRoleProject($event, item, viewModel.LProject[item.GroupId], false)"
                                     :disabled="!isSystemAdmin && !isMantisAdmin">
                          Giám sát
                        </el-checkbox>
                      </div>
      ...
      <el-row>
        <el-col :span="24" class="text-center pt-2">
...
export default {
  ...
  methods: {
    handleRoleChange(checked, item, id) {       
      var ctrl = this;
      var isChecked = false;
      $.each(item.children, (key, ele) => {
        ctrl.viewModel.GroupRoles[ele.GroupId] = (ele.GroupId == id && checked)
       })
        $.each(item.children, (key, ele) => {         
    ...
    checkGroup(checked, item) {
      var ctrl = this;
      if(!checked) {
          $.each(item.children, (key, ele)=> {
             ctrl.viewModel.GroupRoles[ele.GroupId] = checked
          })
    ...
    saveProjectPermiss(userId) {
      ...
      $.each(lstProject, function(k, project) {
        ...
        request.R1_RequestFields = "GroupId;Worker;Manager;Lead;";
    ...
    getAndBindProjects() {
      ...
      request.R1_RequestFields =
        "GroupId;GroupName;GroupType;GroupParent;ProjectCode;CreatedBy;State;Status;Type;Department;ParentName;Description;";
      request.R1_GroupType = "4;5";
      if (!this.$isSystemAdmin() && !this.$isMantisAdmin()) {
        request.R1_AssignedUser = this.$getItemLocalStorage(
          this.$localStorageConstant.SessionId
        );
      }
      this.$Utils.post(request).then(respond => {
        ctrl.model.Groups = ctrl.model.Groups ? ctrl.model.Groups : "";
        respond = this.$Utils.getDataWithRoot(respond.R1, "Data.UserDS.Group");
        ctrl.Project = [];
        ctrl.Roles = [];
        if(respond)
        respond.forEach(ele => {
