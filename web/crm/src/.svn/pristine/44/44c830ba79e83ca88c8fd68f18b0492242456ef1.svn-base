<template>
<el-form >
  <el-form-item label="Chọn kịch bản">
     <el-select v-model="Calculator" clearable value-key="Id" placeholder="Chọn" >
            <el-option
...
export default {
  ...
  methods: {
    Calculate() {
      ...
      var requestCheckExits = this.$Lodash.cloneDeep(this.$singleRequest);
      var requestInsert = this.$Lodash.cloneDeep(this.$singleRequest);
      if (
        ctrl.Calculator.Description &&
        ctrl.Calculator.Description != "" &&
        ctrl.Calculator.Description.constructor === String
      ) {
        var expression = JSON.parse(ctrl.Calculator.Description);
        //console.log(expression.sources);
        if (expression) {
          if (expression.relativeType === "n-1") {
            if (expression.sources) {
              request.TotalRequests = expression.sources.length;
              var i = 1;
              expression.sources.forEach(element => {
                // console.log(element)
                request["R" + i + "_RequestDataGroup"] =
                  element["RequestDataGroup"];
                request["R" + i + "_RequestTemplate"] =
                  element["RequestTemplate"];
                request["R" + i + "_RequestFields"] = element["RequestFields"];
                request["R" + i + "_Code"] = element["Code"];
                i++;
              });
              //Search source config
              //console.log(request);
              utilsLibrary.post(request).then(datas => {
                //console.log(datas);
                var returnSources = [];
                if (datas.success) {
                  var i = 1;
                ...
                if (expression.targets && expression.targets[0]) {
                  var target = expression.targets[0];
                  var inserProducts = {};
                  //console.log(target.CheckExistSourceIndex);
                  var warehouses = utilsLibrary.getDataWithRoot(
                    datas["R" + target.CheckExistSourceIndex1],
                    target["DataRoot"]
                  );
                  console.log('warehouse', warehouses)
                  var products = utilsLibrary.getDataWithRoot(
                    datas["R" + target.CheckExistSourceIndex2],
                    target["DataRoot"]
                  );
                  ...
                  if (target.CheckExistCode1 && target.CheckExistCode2) {
                    warehouses.forEach (warehouse => {
                      products.forEach(product => {
                        j++;
                        requestCheckExits["R" + j + "_RequestDataGroup"] =
                          target["RequestDataGroup"];
                        requestCheckExits["R" + j + "_RequestTemplate"] =
                          target["RequestTemplate"] + "." + "Count";
                        requestCheckExits["R" + j + "_RequestFields"] = target.CheckExistCode1+";"+target.CheckExistCode2 + ";" +target["Code"];
                        requestCheckExits["R" + j + "_Code"] = target["Code"];
                        requestCheckExits["R" + j + "_" + target.CheckExistCode1] =  warehouse[target.CheckExistSourceField1];
                        requestCheckExits["R" + j + "_" + target.CheckExistCode2] =  product[target.CheckExistSourceField2];  
                        
                      });
                    });
                  }
                }
                 requestCheckExits.TotalRequests = j;
                //Check if item has in warehouse or not
                console.log("requestCheckExits ", requestCheckExits);
                utilsLibrary.postCheckResult(requestCheckExits).then(counter => {
                  var v = 1;
                  if (counter.success) {
                    warehouses.forEach (warehouse => {
                      products.forEach(product => {
                        var coutdata = counter["R" + v].Data;
                        var CheckExistValue1 =
                          requestCheckExits[
                            "R" + v + "_" + target.CheckExistCode1
                          ];
                        var CheckExistValue2 =
                          requestCheckExits[
                            "R" + v + "_" + target.CheckExistCode2
                          ];
                        if(coutdata === "0")
                        inserProducts[CheckExistValue1 + "_" + CheckExistValue2] = coutdata;
                        v++;
                      });
                    });
                    var totalRequest = 1;
                    console.log(inserProducts)
                    if (expression.actionType === "Insert") {
                      returnSources[0].forEach(s1 => {
                        returnSources[1].forEach(s2 => {
                          if (
                            inserProducts[s1[target.CheckExistSourceField1] +"_"+ s2[target.CheckExistSourceField2]]
                          ) {
                            requestInsert[
                              "R" + totalRequest + "_RequestDataGroup"
                            ] =
                              target["RequestDataGroup"];
                            requestInsert[
                              "R" + totalRequest + "_RequestTemplate"
                            ] =
                              target["RequestTemplate"] +
                              "." +
                              expression.actionType;
                            requestInsert[
                              "R" + totalRequest + "_RequestFields"
                            ] =
                              target["RequestFields"];
                            requestInsert["R" + totalRequest + "_Code"] =
                              target["Code"];
                             target.binding.forEach( binding => {
                             var key = binding.targetField;
                          var value = undefined;
                          if (binding.defaultValue != "")
                            value = binding.defaultValue;
                          else if (binding.sourceIndex === 1)
                            value = s1[binding.sourceFileld];
                          else if (binding.sourceIndex === 2)
                            value = s2[binding.sourceFileld];
                           console.log(key)
                           console.log(value)
                           if(key && value) {
                               requestInsert[
                              "R" + totalRequest + "_" + key
                             ] = value;
                             } else {
                               return;
                             }
                            })
                            //if(totalRequest === 10) break
                            totalRequest++;
                          } else {
                            return
                          }
                      ...
                      requestInsert.TotalRequests = totalRequest;
                      //requestInsert.TotalRequests = 10;
                      console.log(requestInsert);
                      console.log(
                        "TotalRequests ",
                        requestInsert.TotalRequests
                      );
                     // return;
                      utilsLibrary.postCheckResult(requestInsert).then(result=>{
                        console.log(result)
                      })
    ...
    createCalculatorRequest(data, index) {
      ...
      for (var i = 1; i <= ctrl.objSetting.R; i++) {
        childRequest["R" + i + "_RequestTemplate"] =
          ctrl.objSetting["R" + i + "RequestTemplate"] + ".Search";
        childRequest["R" + i + "_RequestDataGroup"] =
          ctrl.objSetting["R" + i + "RequestDataGroup"];
        childRequest["R" + i + "_RequestFields"] =
          ctrl.objSetting["R" + i + "RequestFields"];
        childRequest["R" + i + "_Code"] = ctrl.objSetting["R" + i + "Code"];
        childRequest.TotalRequests++;
        $.each(
          ctrl.objSetting["R" + i + "RequestFields"].split(";"),
          (key, field) => {
            if (
              utilsLibrary.isEmpty(ctrl.objSetting["R" + i + field]) &&
              field != "Code"
            ) {
              childRequest["R" + i + "_" + field] =
                value[ctrl.objSetting["R" + i + field]];
            }
          }
        );
        if (
          utilsLibrary.isEmpty(ctrl.objSetting["R" + i + "SearchField"]) &&
          !utilsLibrary.isEmpty(
            childRequest[
              "R" + i + "_" + ctrl.objSetting["R" + i + "SearchField"]
            ]
          )
        ) {
          childRequest[
      ...
      if (index == ctrl.rootData.length - 1) {
        ...
        $.each(ctrl.calculatorRequest, (keyRequest, cRequest) => {
          ctrl.calculatorProcessing(
            cRequest,
            ctrl.rootData[keyRequest],
            keyRequest
          );
        });
      }
    ...
    calculatorProcessing(childRequest, value) {
      ...
      utilsLibrary.post(childRequest).then(childData => {
        for (var i = 1; i <= ctrl.objSetting.R; i++) {
          ctrl["DataR" + i] = utilsLibrary.getDataWithRoot(
            childData["R" + i],
            ctrl.objSetting["R" + i + "DataSource"]
          );
          $.each(ctrl["DataR" + i], (key1, childField) => {
            ...
            $.each(ctrl.CountFields, (key, count) => {
              if (
                count.indexOf("!!") >= 0 &&
                !utilsLibrary.isEmpty(compareValue)
              ) {
                var compareValue = "";
              }
              if (utilsLibrary.isEmpty(ctrl.objSetting["R" + i])) {
                $.each(ctrl.objSetting["R" + i].split(";"), (key2, CFields) => {
                  if (
                    count.indexOf("!!") == -1 &&
                    utilsLibrary.isEmpty(childField[count]) &&
                    CFields == key
                  ) {
                    value[key] = parseInt(value[key]) + 1;
                  } else if (
                    utilsLibrary.isEmpty(compareValue) &&
                    utilsLibrary.isEmpty(childField[count.split("!!")[0]]) &&
                    CFields == key
                  ) {
                    if (
                      compareValue.indexOf(childField[count.split("!!")[1]]) ==
                      -1
                    ) {
                      value[key] = parseInt(value[key]) + 1;
            ...
            $.each(ctrl.AverageFields, (key, average) => {
              ...
              if (utilsLibrary.isEmpty(childField[average])) {
                value[key].sum =
                  parseInt(value[key].sum) + parseInt(childField[average]);
              }
              if (utilsLibrary.isEmpty(ctrl.objSetting["R" + i])) {
                $.each(ctrl.objSetting["R" + i].split(";"), (key4, CFields) => {
                  if (
                    utilsLibrary.isEmpty(childField[average]) &&
                    CFields == key
                  ) {
        ...
        if (utilsLibrary.isEmpty(ctrl.objSetting.ExtraFields)) {
          ...
          var valueExtra = utilsLibrary.stringToObject(
            ctrl.objSetting.ExtraValue.replaceAll("+", "&").replaceAll(":", "=")
          );
          $.each(valueExtra, (KExtra, VExtra) => {
            if (KExtra == value[keyExtra[0]]) {
              ...
              var Extra = 0;
              $.each(
                ctrl["Data" + ctrl.objSetting.ExtraRequest],
                (key5, childField) => {
                  if (
                    utilsLibrary.isEmpty(
                      childField[ctrl.objSetting.ExtraCondition]
                    )
                  ) {
              ...
              var valueExtraAfter = utilsLibrary.stringToObject(
                ctrl.objSetting.ExtraValueAfter.replaceAll("+", "&").replaceAll(
                  ":",
                  "="
                )
              );
              ...
              if (
                utilsLibrary.isEmpty(valueExtraAfter.ELSE) &&
                value[keyExtra[1]] == 0
              ) {
          ...
          if (
            utilsLibrary.isEmpty(valueExtra.ELSE) &&
            value[keyExtra[1]] == 0
          ) {
        ...
        $.each(value, (updateKey, updateField) => {
          if (
            utilsLibrary.isEmpty(updateField, "sum") &&
            utilsLibrary.isEmpty(updateField, "count")
          ) {
        ...
        $.each(ctrl.ExpressionFields, (key, expression) => {
          ...
          while (exp != "") {
            if (
              ...
              var name = this.$Lodash.cloneDeep(ekey),
                evalue = "";
              if (ekey.indexOf(".") > 0) {
        ...
        objRequest.RequestTemplate =
          ctrl.objSetting.RRequestTemplate + ".Update";
    ...
    init() {
      ...
      request.R1_ParentCode = "xSetting.Project.Tools.Warehouse";
      utilsLibrary.post(request).then(data => {
        ctrl.comboCalculator = utilsLibrary.getDataWithRoot(
          data.R1,
