...
export default {
    ...
    methods: {
      ...
      manualUpdate(){
        ...
        // request.R1_Lead = 'A0075;A0098;A0117;A0118;A0119;A0120;D0089;D0109;D0110;D0111;D0112;D0115;D0116;D0122;J0121;Q0097;S0108'
        // request['R2_Parent'] = val.Id;
        // request['R1_Promulgator'] = objPos[item.Store];
        request['R1_RequestFields'] = "Id;Targets;TargetsName";
        request.R2_RequestTemplate = 'Setting.SearchBase';
        request.R2_ParentCode = 'xSetting.Project.Parameter.Organization.Target';
        // request.R1_NotNull = 'Targets'
        ...
        // request.R2_ParentCode = 'xSetting.Project.Parameter.CRM.Product.Size';
        ...
        // this.$Utils.post(request).then(data => {
          ...
          this.$Utils.post(request).then((data) => {
            // var arrPages = this.$Utils.getDataWithRoot(data.R1 ,"Data.DocumentDS.Document");
            var arrChild = this.$Utils.getDataWithRoot(data.R1 ,"Data.TasksDS.Task");
            arrChild = arrChild.filter(el => {
              if(el.Targets && el.Targets != '') {
                el.TargetsName = el.Targets;
                return el.Targets && el.Targets != ''
              } 
            })
            var arrParams = this.$Utils.getDataWithRoot(data.R2 ,"Data.DynamicDS.Setting");
            arrChild.forEach(item => {
              arrParams.forEach(val => {
                if(item.Targets.indexOf(val.Id)!= -1) {
                  item.TargetsName = item.TargetsName.replaceAll(val.Id, val.Name)
                }
              })
            })
            //console.log(arrChild)
            // arrParams = this.$Lodash.chain(arrParams).keyBy('Id').value();
          // });
          ...
          // console.log(arrParams)
            // var Count = 0;
             arrChild.forEach(item => {
            //   // if(item.Size && item.Size !='' && !item.SizeName){
            //     // Count ++;
            //     // console.log(item.Size)
                var upRequest = this.$Lodash.cloneDeep(this.$singleRequest);
                upRequest['R1_RequestTemplate'] =  'AG_Task_Task.Update';
                upRequest['R1_RequestDataGroup'] = 'DataGroup.danh-sach-cong-viec.04b66';
                upRequest['R1_Id'] = item.Id;
                upRequest['R1_TargetsName'] = item.TargetsName;
                upRequest['R1_RequestFields'] = "Id;TargetsName";
      ...
      init() {
        ...
        this.$Utils.post(request).then(data => {
          ...
          if(this.$Utils.isEmpty(this.paramCode) && this.paramCode.trim() != '' && tree.length > 0) {
