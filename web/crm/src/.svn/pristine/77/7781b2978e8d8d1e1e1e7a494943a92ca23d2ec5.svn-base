        ...
        <el-container>
          <el-header>
            <el-form :inline="true" v-if="!$isMobileDevice">
              ...
              <el-form-item label="Phòng ban" v-if="!typeproject">
                <el-select v-model="derparment" placeholder="Phòng ban">
                  <el-option v-for="item in Projects" :key="item.GroupId" :label="item.GroupName"
                             :value="item.GroupId"></el-option>
                </el-select>
            ...
            <div class="bg-white pb-1" v-if="isCollapseFilter">
              <el-form label-position="right" size="mini" label-width="120px">
                ...
                <el-form-item label="Phòng ban" style="width:100%;" v-if="!typeproject">
                  <el-select v-model="derparment" placeholder="Phòng ban">
                    <el-option v-for="item in Projects" :key="item.GroupId" :label="item.GroupName"
                               :value="item.GroupId"></el-option>
                  </el-select>
...
export default {
  ...
  methods: {
    ...
    onSearch() {
      // document.getElementById("mySidenav").style.width = "0";
      if(this.typeproject == true && this.derparment == 1)
    ...
    changeRadio(item) {
      ...
      if (item === 1) {
        ...
        this.$Utils.post(request).then(response => {          
          var data = this.$Utils.getDataWithRoot(
          response.R1,'Data.DynamicDS.Setting');
          if(!this.$isSystemAdmin() && !this.$isMantisAdmin()){           
            var depIds =  this.currentUser.Groups.map(d=> {
              return d.GroupId;
            })
            data = this.$Lodash.filter(data, cat=> {
                return this.$Lodash.intersection(depIds, cat.Value.split(";")).length > 0
            })
          }        
