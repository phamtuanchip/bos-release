...
<div class="tasklist">
  ...
  <el-row >
    <el-col :span="22" v-if="!Setting.IsDetail">
      <el-tooltip class="item" effect="dark" :content="Setting.InsertButton.Caption" placement="top" v-if="Setting.ShowInsertButton">
         <el-button @click="onLinkClick(Setting.InsertButton,{})"><i class="el-icon-circle-plus-outline"></i></el-button>
      </el-tooltip>
      <span v-if="$isMobileDevice || (!Setting.ShowInsertButton && !Setting.ShowExportButton)">&nbsp;</span>
      <el-tooltip class="item" effect="dark" content="Xuất excel" placement="top"  v-if="Setting.ShowExportButton && !$isMobileDevice">
  ...
  <el-table :data="FormatData" style="width: 100%" border empty-text="Không có dữ liệu" :show-summary="Setting.TotalRow == 'true'" sum-text="Tổng" :summary-method="getSummaries">
    ...
    <template v-for="(item, index) in ListColumns">
      ...
      <el-table-column :key="index" :label="item.Caption" :class-name="formatClass(item)" :min-width="item.Caption === 'Công việc' ? '300' : '80'" v-if="Setting.DisplayType !== 'Tree' || (Setting.DisplayType == 'Tree' && (item.Name !== Setting.TreeCaptionKey && item.Name !== Setting.TreeCaptionKey + 'Name'))">
        <template slot-scope="scope">
          <div :style="formatStyle(item, scope.row)">
            <div v-if="item.Name === 'Tool'">
                ...
                <div layout="row" class="default-tool" v-else>
                  ...
                  <span class="position-relative pr-1" v-for="(button, bIndex) in ListButtons" :key="button.Id"  v-if="(button.ExtraTextClass && displayControl(button, scope.row).extraText) || !button.ExtraTextClass">
                      ...
                      -->
                      <a v-if="button.ButtonType != 'Redirect'" @click="onButtonClick(bIndex, scope.row)"><i type="primary" :class="'icon ' + button.Icon" data-toggle='tooltip' data-placement='top' :title='button.Caption'></i>
                        <span class="bg-danger text-white text-center" v-if="button.ExtraTextClass">{{displayControl(button, scope.row).extraText}}</span>
                      </a>
                  </span>
                </div>
            ...
            <el-progress v-else-if="item.ProgressBar == 'true'" :text-inside="true" :stroke-width="18" :percentage="!scope.row[item.Name] ? 0 : scope.row[item.Name] <= 100 ? parseInt(scope.row[item.Name])  : 100"></el-progress>
            <div clas="row" v-else-if="item.ElementType == 'Image'">
              <span v-for="(ava, avaIndex) in getImageObj(scope.row, item)" class="col-md-6">
                <img class = 'avatar' style='margin: 8px 10px 0 0;border-radius: 50em;' height='35' width='35' :title="avaIndex"
      ...
      <el-table-tree-column :prop="item.Name" :key="index" :label="item.Caption" :class-name="formatClass(item)" :min-width="300" v-if="(Setting.DisplayType == 'Tree' && item.Name === Setting.TreeCaptionKey) || (Setting.DisplayType == 'Tree' && item.OldName === Setting.TreeCaptionKey)" :treeKey="Setting.TreeKey" :parentKey="Setting.TreeParent" :childKey="Setting.TreeChild" :expandAll="Setting.TreeExpandAll != 'true'" :show-overflow-tooltip="true" levelKey="Level">
        <template slot-scope="scope">
...
export default {
  ...
  data() {
    ...
    ctrl.endIndex = 50;
    ctrl.totalP = 0;
    ctrl.maxItem = ctrl.Setting.LimitIndex ? ctrl.Setting.LimitIndex : 50;
    ctrl.dateStart = '';
    ctrl.dateEnd = '';
    ctrl.selectedWorker = [];
